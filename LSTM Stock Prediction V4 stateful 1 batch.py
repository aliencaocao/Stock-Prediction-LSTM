...
DROPOUT = [0.000, 0.000, 0.00, 0.003]
UNITS = [512, 1024, 256, 256]
BATCH_SIZE = 32
EPOCH = 1  # how many times to run at once
...
def build_model():
    model.add(LSTM(units=UNITS[0], activation=ACTIVATION, return_sequences=True,
                   ...
                   kernel_initializer=initializer, stateful=True,
    ...
    model.add(LSTM(units=UNITS[1], activation=ACTIVATION, return_sequences=True, kernel_initializer=initializer,
                   recurrent_activation=RECURRENT_ACTIVATION, recurrent_dropout=0, stateful=True,
...
y_predict = model.predict(X_test.reshape(1, -1, 5))
...
print(y_predict_normalized[-1])
# Prediction of actual price based on predicted movement %
predicted_change_pct = float(
    y_predict_normalized[-1][-1] / y_predict_normalized[-1][-2])
last_predicted_change_pct = float(
    y_predict_normalized[-1][-2] / y_predict_normalized[-1][-3])
...
plt.plot(y_test_normalized[-1], color='red', label="Actual")
plt.plot(y_predict_normalized[-1], color='green', label="Prediction")
